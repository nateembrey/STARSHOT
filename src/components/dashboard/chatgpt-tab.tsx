...
export function ChatGptTab() {
  ...
  const handleSend = () => {
    if (input.trim()) {
      ...
      // Mock bot response
      setTimeout(() => {
        setMessages(prev => [...prev, { sender: "bot", text: `This is a mocked ChatGPT response for: "${input}"` }]);
      }, 1000);
    }
  };
  ...
  return (
    <Card className="h-[75vh] flex flex-col">
        <CardHeader>
            <h2 className="text-2xl font-bold">Chat with ChatGPT</h2>
        </CardHeader>
          ...
          <div className="space-y-4">
            {messages.map((message, index) => (
              <div key={index} className={`flex items-start gap-4 ${message.sender === 'user' ? 'justify-end' : ''}`}>
                {message.sender === 'bot' && <Bot className="h-8 w-8 text-primary" />}
