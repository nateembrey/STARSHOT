...
} from "@/ai/flows/predict-profit-flow";
            ...
            <CardContent className="p-6 pt-0 md:p-8 md:pt-0">
                <div className={cn(!isLoading && "animate-content-in")}>
                    {isLoading ? (
                        ...
                        <>
                            <div className="text-4xl md:text-6xl font-bold tracking-tight text-muted-foreground/50">
                                N/A
                            </div>
                            {subtext && (
...
const ClosedTradesTable = ({
    ...
    return (
        <Card className="card-interactive">
            ...
            <CardContent
                className={cn(
                    "p-6 pt-0 md:p-8 md:pt-0",
                    !isLoading && "animate-content-in",
                )}
            >
                {isLoading ? (
                                        ...
                                        <TableRow key={index}>
                                            ...
                                            <TableCell className="text-right font-semibold whitespace-nowrap text-xs">
                                                <span className={colorClass}>
                                                    {trade.profitAbs?.toLocaleString(
                                                        "en-US",
                                                        {
                                                            style: "currency",
                                                            currency: "USD",
                                            ...
                                            <TableCell className="text-right font-semibold whitespace-nowrap text-xs">
                                                {trade.profitPercentage?.toFixed(
                                                    1,
                                                )}
                                                %
                                            </TableCell>
                ...
                {hasData && (
                    <div className={cn(!isLoading && "animate-content-in")}>
                        ...
                        <p className="font-bold text-lg text-[hsl(var(--chart-2))]">
                            {biggestWin.toLocaleString("en-US", {
                                style: "currency",
                                currency: "USD",
                            })}
                        </p>
...
const CumulativeProfitChart = ({
    ...
    const [prediction, setPrediction] = useState<
        PredictProfitOutput["prediction"] | null
    >(null);
    const [predictionDuration, setPredictionDuration] =
        useState<PredictionDuration>("1M");
        ...
        try {
            ...
            if (historyForPrediction.length < 2) {
                toast({
                    ...
                    description:
                        "At least 2 closed trades are required to make a prediction.",
                });
                setIsPredicting(false);
                return;
            }

            const result = await predictProfit({
    ...
    const finalProfit =
        data.length > 0 ? (data[data.length - 1].cumulativeProfit ?? 0) : 0;
    ...
    const canPredict =
        hasData && data.filter((d) => d.name !== "Start").length >= 2;

    return (
        <div className="relative perspective h-[420px] md:h-[364px]">
            <div
                className={cn(
                    "w-full h-full transform-style-3d transition-transform duration-700",
                ...
                <Card className="absolute w-full h-full backface-hidden card-interactive">
                    <CardHeader className="p-6 md:p-8 flex flex-row items-start justify-between">
                        <div>
                            ...
                            <CardDescription className="text-xs">
                                Growth of total profit over time.
                            </CardDescription>
                        </div>
                        <Button
                            variant="outline"
                                                    ...
                                                    if (
                                                        ...
                                                        return (
                                                            <div className="flex flex-col gap-1">
                                                                ...
                                                                <span
                                                                    className={
                                                                        payload.cumulativeProfit >=
                                                                        0
                                                                            ? "text-[hsl(var(--chart-2))]"
                                                                            : "text-[hsl(var(--accent))]"
                                                                    }
                                                                >
                                                                    {payload.cumulativeProfit.toLocaleString(
                                                                        "en-US",
                                                                        {
                                                                            style: "currency",
                                                                            currency:
                ...
                <Card className="absolute w-full h-full backface-hidden rotate-y-180 card-interactive">
                    ...
                    <CardContent className="p-6 pt-0 md:p-8 md:pt-0">
                        <div className="flex items-center justify-center space-x-2 mb-4">
                            {(
                                ["1W", "1M", "3M", "1Y"] as PredictionDuration[]
                            ).map((d) => (
                                <Button
                                    key={d}
                                    size="sm"
                                    variant={
                                        predictionDuration === d
                                            ? "secondary"
                                            : "ghost"
                                    }
                                ...
                                >
                                    {isPredicting && predictionDuration === d
                                        ? "..."
                                        : d}
                                </Button>
                            ))}
                        ...
                        {isPredicting ? (
                            ...
                            <div className="flex items-center justify-center h-[168px] text-muted-foreground text-sm text-center px-4">
                                {canPredict
                                    ? "Select a duration to generate a forecast."
                                    : "At least 2 closed trades are required to generate a forecast."}
                            </div>
                        ) : (
                            <ChartContainer
                                config={{
...
export function DashboardTab({
    ...
    const hasData =
        !!data && (data.totalTrades > 0 || data.openTrades.length > 0);
    const pnlValue = data?.pnl ?? 0;
    const pnlColor =
        pnlValue >= 0
            ? "text-[hsl(var(--chart-2))]"
            : "text-[hsl(var(--accent))]";
    const winsLossesIcon =
        (data?.winningTrades ?? 0) >= (data?.losingTrades ?? 0)
            ? ChevronsUp
            : ChevronsDown;
    const percentageProfitValue = data?.percentageProfit ?? 0;
    const percentageProfitColor =
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
                <StatCard
                    isLoading={isLoading}
                    hasData={!!data && data.totalTrades > 0}
                    title="Total P&L"
                    value={
                        <span className={pnlColor}>{formatPnl(pnlValue)}</span>
                    }
                    icon={TrendingUp}
                    subtext="Profit & Loss from closed trades."
                />
                <StatCard
                    isLoading={isLoading}
                    hasData={!!data && data.totalTrades > 0}
                    ...
                    value={
                        <span
                            className={percentageProfitColor}
                        >{`${percentageProfitValue.toFixed(2)}%`}</span>
                    }
                    icon={TrendingUp}
                    subtext="Total P&L / total invested capital."
                />
                <StatCard
                    isLoading={isLoading}
                    hasData={!!data && data.totalTrades > 0}
                    title="Closed Trades"
                    value={data?.totalTrades.toLocaleString() ?? "N/A"}
                    icon={Activity}
                    subtext="Total trades completed."
                />
                <StatCard
                    isLoading={isLoading}
                    hasData={!!data && data.totalTrades > 0}
                ...
                <StatCard
                    ...
                    value={
                        <span>
                            <span className={winsLossesColor}>{wins}</span>
                            <span
                                className={cn(
                            ...
                            <span className={winsLossesColor}>{losses}</span>
                        </span>
                    }
                    icon={winsLossesIcon}
                    subtext="Profitable vs. unprofitable trades."
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <ProfitLossChart
                    ...
                    isLoading={isLoading}
                    hasData={
                        !!data && (data?.tradeHistoryForCharts?.length ?? 0) > 0
                    }
                    biggestWin={data?.biggestWin ?? 0}
                />
                <CumulativeProfitChart
                    data={data?.cumulativeProfitHistory ?? []}
                    isLoading={isLoading}
                    hasData={
                        !!data &&
                        (data?.cumulativeProfitHistory?.length ?? 0) > 1
                    }
                />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {/* START: Updated Open Trades Card */}
                <Card className="card-interactive">
                    <CardHeader className="p-6 md:p-8">
                        <CardTitle className="text-lg md:text-xl">
                            Open Trades
                                    ...
                                    {openTrades.map((trade: Trade) => {
                                        ...
                                        return (
                                            <TableRow key={trade.trade_id}>
                                                ...
                                                <TableCell
                                                    className={`text-right font-semibold whitespace-nowrap text-xs ${colorClass}`}
                                                >
                                                    {trade.profitAbs.toLocaleString(
                                                        "en-US",
                                                        {
